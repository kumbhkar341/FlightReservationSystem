...
public class CreditCardDao {
	...
	public int save(CreditCard p){    
	    String sql="insert into frs_tbl_creditcard(CreditCardNumber,ValidFrom,ValidTo,UserId) values('"+p.getCreditCardNumber()+"','"+p.getValidFrom()+"','"+p.getValidTo()+"','"+p.getUserId()+"')";    
	    return template.update(sql);    
	}
	public CreditCard getCredByNum(int CreditCardNumber){  
	    String sql="select * from  frs_tbl_creditcard where  CreditCardNumber=?";  
	...
	{
		String sql="delete from  frs_tbl_creditcard where  CreditCardNumber="+CreditCardNumber+"";
		return template.update(sql);

}
	public List<CreditCard> getCard(int x){    
	    return template.query("select * from frs_tbl_creditcard where CreditCardNumber="+x+"",new RowMapper<CreditCard>(){    
	...
	{
		String sql="select faree from frs_tbl_route where routeid=?";
	...
	{
		String sql="update frs_tbl_creditcard set balance=balance-"+i+" where creditcardnumber="+cr+"";
		return template.update(sql);

}
	public int updateps(int route,int y,int z) {
		String tf = (String) this.template.queryForObject( "select faree from frs_tbl_route where routeid = ?", 
		        new Object[]{route}, String.class);
		String bal=(String) this.template.queryForObject( "select balance from frs_tbl_CREDITCARD where creditcardnumber = ?", 
		...
		String sql="update frs_tbl_reservation set bookingstatus='Paid' where userid='"+z+"'";
		String sql2="update frs_tbl_creditcard set balance="+newbl+" where creditcardnumber='"+y+"'";
